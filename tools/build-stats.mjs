...
/* ── 발생장 묶음 — 지금은 **묶지 않습니다** (2026-09-07 AURI 확인) ──────
   ...
   묶었습니다. 그런데 AURI 우 연구원이 이 이름이 **틀린 표현**이라고 확인했습니다.

       "용도지역상 '교통 지역'이라는 명칭은 없다. 잘못된 표현이다."

   사전진단서 원문에 그렇게 적혀 있어도 따라가면 우리 문서까지 틀린 말을 하게
   되므로, **원자료에 적힌 값을 그대로** 씁니다 — `도로` 와 `도로외교통지역` 을
   각각 표기합니다.

   표를 비워 두면 묶기가 저절로 꺼지고, `placeGrouped` 는 원본과 같아집니다.
   읽는 쪽(stats.js·rules.js)은 고칠 필요가 없습니다.

   ⏸ 다시 묶으려면 — AURI 가 보내기로 한 **분류 기준 자료**가 도착해 올바른
      상위 명칭이 정해진 뒤, 아래에 `'도로': '<새 이름>'` 꼴로 넣으면 됩니다.
      기준 없이 우리가 이름을 지어내지 않습니다.                        */
const PLACE_GROUPS = {
};
