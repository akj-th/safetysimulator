...
/* ── 신규 / 기존 구분 ──────────────────────────────────────────────
   시트마다 표가 두 덩어리 쌓여 있고, 위쪽만 셀에 배경색이 칠해져 있습니다.
     ...
     칠해진 쪽 (DCE6F2 파랑 · F2DCDB 분홍)  → 기존 사업 팔레트
     칠하지 않은 쪽 (아래 표)                → 신규 (비중을 차등화해 새로 정리)

   ★ 근거: AURI 우제헌 연구원 메일 (2026-09)
     "색이 있는 부분은 기존 사업팔레트이며, 그 아래 색이 없는 부분은
      비중을 차등화하여 새롭게 정리한 팔레트입니다."

   ※ 처음에는 반대로 보았습니다. 칠해진 표가 머리행 바로 아래 오는 본 표이고,
     금액 기입률도 높으며, 요약 시트의 "A 과잉·E 부족" 지적과 견주면 칠해진
     쪽이 E 비중이 높아 보정본처럼 읽혔기 때문입니다. 자료만으로는 판단이
     갈리는 부분이라, 메일에 적힌 대로 따릅니다.

   ⚠️ 다시 뒤집어야 하면 이 상수만 바꾸면 됩니다.                        */
const COLORED_IS_NEW = false;
