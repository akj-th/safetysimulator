...
/* ② 합산·평활을 마친 종합 지수를 0~1로 환산할 때 (auriCompositeField)
   ...
   변경 이력
   - 2026-09-07  0.99 확정 — 격자 지도 색 분포만 보고 정함
   - 2026-09-15  0.99 → 0.97 (담당자 확정)
       종합 위험도 가중치가 사진 10 : 이력 90 이 되면서 종합 판정이 이 지수에 거의 전적으로
       의존하게 되어, 격자 지도만이 아니라 **종합 판정 분포까지 함께** 보고 다시 정한 값.
       부천·미추홀 실측(이력이 있는 격자, 안전·주의·위험 %):
         0.99  격자 70·25·5 / 78·18·3   종합(사진 72) 62·33·6 / 72·25·4  ← 사진이 위험이어도 종합 안전 과반
         0.98  격자 61·31·8 / 69·25·6   종합(사진 72) 52·39·9 / 62·32·6  ← 여전히 종합 안전 과반 → 부적절
         0.97  격자 55·35·10 / 62·29·9  종합(사진 72) 46·43·11 / 53·37·10 ← 채택
         0.96  격자 50·37·13 / 53·34·14 종합(사진 72) 41·45·13 / 44·41·14 ← 지도가 과하게 붉어져 9/7 판단과 멀어짐
       ※ 이 값은 지도 색과 종합 위험도에만 관여합니다. 처방 규칙·현장 확인사항은 사진 판독 항목만 봅니다. */
const RISK_INDEX_QUANTILE = 0.97;
...
/* 종합 위험도를 낼 때 사진 판독 점수가 차지하는 비중 (나머지가 주변 이력).
   ...
   변경 이력
   - 2026-08 ~ 2026-09-15  사진 70 : 이력 30 (RISK_BLEND_PHOTO_2026_08)
       처방이 손대는 것은 사진에 찍힌 물리적 대상이므로 사진 쪽을 무겁게 둔다는 판단.
   - 2026-09-16 ~          사진 10 : 이력 90 (RISK_BLEND_PHOTO_2026_09_15)
       9/15 행안부 국장 보고 현장 의견: "사진 판독은 선택한 지점 한 곳의 국소적 상태만 보는데
       70%는 과하다. 축적된 통계 쪽에 무게를 두어야 한다." → 주변 이력 90.
   되돌리려면 RISK_BLEND_PHOTO 가 가리키는 상수만 바꾸면 됩니다.
   ⚠️ 항목별 점수와 개선사업 처방은 이 값과 무관합니다(사진 판독 결과만으로 정함).
      바뀌는 것은 리포트·서식1의 "종합 위험도" 숫자와 판정뿐입니다. */
const RISK_BLEND_PHOTO_2026_08 = 0.7;      // 옛 값 — 되돌릴 때 사용
const RISK_BLEND_PHOTO_2026_09_15 = 0.1;   // 현재 값 — 9/15 행안부 보고 의견 반영
const RISK_BLEND_PHOTO = RISK_BLEND_PHOTO_2026_09_15;
